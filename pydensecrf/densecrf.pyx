...
cdef class DenseCRF:
    ...
    def gradient(self, int niter, label, float robust, float[:] unary_grad, float[:] cmp_grad, float[:] kernel_grad):
        return self._this.gradient(niter, _objectfunc(label, robust), &unary_grad[0], &cmp_grad[0], &kernel_grad[0])
